import { Gender } from "@app/modules/users/dtos/gender.enum";
import { Role } from "@app/modules/users/dtos/role.enum";
...
export function getGenderNumber(gender?: Gender): number | undefined {
  if (!gender) {
    return undefined;
  }
...
export function getRoleNumber(role?: Role): number | undefined {
  if (!role) {
    return undefined;
  }
